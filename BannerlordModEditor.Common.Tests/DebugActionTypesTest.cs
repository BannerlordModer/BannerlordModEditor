...
{
    public class DebugActionTypesTest
    {
        [Fact]
        public void Debug_ActionTypes_Deserialization_Serialization()
        {
            var xmlPath = "TestData/action_types.xml";
            ...
            Console.WriteLine($"Deserialized Actions Count: {obj.Actions?.Count ?? 0}");
            ...
            if (!diff.IsStructurallyEqual)
            {
                ...
                File.WriteAllText("Debug/original_action_types.xml", xml);
                File.WriteAllText("Debug/serialized_action_types.xml", serialized);
            }
        }
    }
}