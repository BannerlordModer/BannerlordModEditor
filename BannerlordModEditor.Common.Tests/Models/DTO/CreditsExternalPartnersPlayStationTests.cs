...
public class CreditsExternalPartnersPlayStationTests
{
    [Fact]
    public void TestXmlSerialization()
    {
        var dto = new CreditsExternalPartnersPlayStationDTO
        {
            Categories = new List<CreditsCategoryDTO>
            {
                new CreditsCategoryDTO
                {
                    Text = "PlayStation Partners",
                    Entries = new List<CreditsEntryDTO>
                    {
                        new CreditsEntryDTO { Text = "Sony Interactive Entertainment" },
        ...
        var deserialized = XmlTestUtils.Deserialize<CreditsExternalPartnersPlayStationDTO>(xml);
...
<Credits></Credits>";
        ...
        var dto = XmlTestUtils.Deserialize<CreditsExternalPartnersPlayStationDTO>(xml);
    ...
    {
        var testDataPath = Path.Combine("TestData", "CreditsExternalPartnersPlayStation.xml");
        if (File.Exists(testDataPath))
        {
            ...
            // 验证往返序列化后的对象仍然正确
            var reDeserialized = XmlTestUtils.Deserialize<CreditsExternalPartnersPlayStationDTO>(serialized);
