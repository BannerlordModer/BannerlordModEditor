...
namespace BannerlordModEditor.Common.Tests.Models.V1_3_15
{
    ...
    {
        [Fact]
        public async Task SettlementTracks_Roundtrip_ShouldPreserveData()
        {
            var xmlPath = Path.Combine("TestData", "V1_3_15", "settlement_tracks.xml");
            var originalXml = await File.ReadAllTextAsync(xmlPath);
            var obj = XmlTestUtils.Deserialize<SettlementTracks>(originalXml);
