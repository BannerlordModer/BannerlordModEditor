...
{
    public class SpKingdomsRoundtripTests
    {
        [Fact]
        public async Task SpKingdoms_Roundtrip_ShouldPreserveData()
        {
            var xmlPath = Path.Combine("TestData", "V1_3_15", "spkingdoms.xml");
            var originalXml = await File.ReadAllTextAsync(xmlPath);
            var obj = XmlTestUtils.Deserialize<SpKingdoms>(originalXml);
